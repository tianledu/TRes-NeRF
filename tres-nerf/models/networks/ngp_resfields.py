...
class NeRFNGPNet(nn.Module):
    def __init__(self, opt):
        ...
        # from instant_avatar.models.nvr.uv_deformer import Deformer
        # from instant_avatar.models.nvr.config import cfg
        # from instant_avatar.models.nvr.make_network import make_deformer
        # cfg.merge_from_file('/home/root_1/xxl/InstantAvatar/confs/network/nvr.yaml')
    ...
    def forward(self, x, d, batch=None):
        embed, res, mu, log_std, resd = 0, 0, 0, 0, torch.zeros_like(x)
        
        # if batch != None:
        #     resd = self.tpose_deformer(x, batch, flag=None)
        #     x = x + resd  # 计算残差init_bigpose
            ...
            # x = x + res.reshape(-1, 3) * 0.1
        
        # normalize pts and view_dir to [0, 1]
        x = (x - self.center) / self.scale + 0.5
        assert x.min() >= -EPS and x.max() < 1 + EPS  # 断言采样点的大小
        x = x.clamp(min=0, max=1)  # 裁剪采样点数值
        ...
        sigma = x[..., 0]
        color = self.color_net(torch.cat([x[..., 1:], torch.sigmoid(sdf_nn_output)*0.01], dim=-1)).float()
        # color += color_ref  # 考虑均值
