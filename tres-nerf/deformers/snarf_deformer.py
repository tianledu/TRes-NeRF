...
class SNARFDeformer():
    ...
    @torch.no_grad()
    def deform_test(self, pts, model, batch):
        ...
        if valid.any():
            with torch.cuda.amp.autocast():
                if batch!=None:
                    batch['pts']=pts
                rgb_cano[valid], sigma_cano[valid], _ = model(pts_cano_all[valid], None, batch)  # 跳转到instant_avatar/models/networks【将采样点输入网络，返回对应颜色和体密度】
    ...
    def deform_train(self, pts, model, batch):
        ...
        if valid.any():
            ...
            with torch.cuda.amp.autocast():
                if batch!=None:
                    batch['pts']=pts
                rgb_cano[valid], sigma_cano[valid], result = model(pts_cano_all[valid], None, batch)  # 输入cano空间采样点到nerf网络
        ...
        sigma_cano, idx = torch.max(sigma_cano, dim=-1)
        rgb_cano = torch.gather(rgb_cano, 1, idx[:, None, None].repeat(1, 1, 3))
        return rgb_cano.reshape(-1, 3), sigma_cano.reshape(-1), result

    def __call__(self, pts, model, eval_mode=True, batch=None):
        if eval_mode:
            return self.deform_test(pts, model, batch)
        else:
            return self.deform_train(pts, model, batch)
